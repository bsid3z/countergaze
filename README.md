...
| Battery percentage (`src/battery.cpp`) | This is the only board here with a battery input — an IP5306 and a cell connector at `P5`. The title bar gains a percentage left of the padlock and rotate icons; every other board reports no battery and draws nothing, so their bar is unchanged. |
