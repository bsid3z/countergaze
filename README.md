...
| Battery percentage (`src/battery.cpp`) | The only board here with a battery input (IP5306, cell connector `P5`). The title bar gains a percentage left of the padlock and rotate icons. Reading its GPIO5 divider has one safe method and one that switches the board off — see [docs/JC3248W535EN.md](docs/JC3248W535EN.md#how-to-read-gpio5-without-switching-the-board-off) before changing it. Other boards report no battery and draw nothing, so their bar is unchanged. |
